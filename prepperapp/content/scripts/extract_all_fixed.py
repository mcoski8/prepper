...
Fixed version for current libzim API
"""
...
class UnifiedZIMExtractor:
    ...
    def extract_article(self, item, priority: int, matched_keywords: List[str], 
        ...
        try:
            # Get article content
            content = item.get_data().decode('utf-8', errors='ignore')
            ...
            # Generate article ID
            article_id = hashlib.md5(f"{priority}:{item.get_title()}".encode()).hexdigest()[:12]
            ...
            print(f"\nError extracting {item.get_title()}: {e}")
            return False
    ...
    def process_zim_file(self, zim_path: str, limit: Optional[int] = None):
        ...
        try:
            # Use libzim.reader.Archive for the reader API
            with libzim.reader.Archive(zim_path) as zim:
                print(f"ZIM file opened. Total entries: {zim.entry_count:,}")
                print(f"  Main entries: {zim.main_entry.get_item().title if zim.has_main_entry else 'None'}")
                
                # Use tqdm for progress bar - iterate through all entries
                for entry in tqdm(zim, desc="Scanning ZIM entries"):
                    ...
                    # Skip non-article entries (check path or mimetype)
                    if not item.mimetype.startswith('text/html'):
                        continue
                    
                    # Get content preview for assessment
                    try:
                        content_preview = item.get_data().decode('utf-8', errors='ignore')[:2000]
                    except:
                        continue
