...
Version 3: Uses correct libzim 3.7.0 API
"""
...
class UnifiedZIMExtractor:
    ...
    def extract_article(self, entry, priority: int, matched_keywords: List[str], 
        ...
        try:
            ...
            # Get article content (read() returns bytes)
    ...
    def process_zim_file(self, zim_path: str, limit: Optional[int] = None):
        ...
        try:
            # Open ZIM file using libzim.reader.Archive
            zim = libzim.reader.Archive(zim_path)
            
            print(f"ZIM file opened. Total entries: {zim.all_entry_count:,}")
            print(f"  Article entries: {zim.article_count:,}")
            print(f"  Media entries: {zim.media_count:,}")
            
            # Get total count for progress bar
            # Note: We'll iterate all entries, not just articles, to maintain stats
            total_entries = zim.all_entry_count
            
            # Use iter_entries_by_id() to iterate through all entries
            # We don't filter by namespace here to maintain redirect stats
            entry_iterator = zim.iter_entries_by_id()
            
            # Use tqdm for progress bar
            for entry in tqdm(entry_iterator, total=total_entries, desc="Scanning ZIM entries"):
                ...
                # Check if it's an article (HTML content)
                # Get the item to check mimetype
                try:
                    item = entry.get_item()
                ...
                if priority >= 0:
                    # Extract article (re-use the entry, don't re-read)
                    if self.extract_article(entry, priority, matched_keywords, 
    ...
    def print_summary(self):
        ...
        if self.stats["errors"] > 0:
            print(f"Extraction errors: {self.stats['errors']:,}")
